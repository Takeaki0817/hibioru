...
#### Acceptance Criteria
1. The 日付ヘッダー shall 記録がある日付をカルーセル形式で表示し、横スクロールで日付を選択できる
2. When ユーザーがカルーセルで日付を選択する, the タイムラインページ shall その日付の投稿位置までスムースにスクロールする
3. When ユーザーが日付ヘッダーのカレンダーアイコンをタップする, the タイムラインページ shall 月カレンダーを展開表示する
4. While 月カレンダーが展開されている, when ユーザーが特定の日付をタップする, the タイムラインページ shall その日付の投稿位置までスクロールする

> **設計変更メモ**: 当初の仕様ではスワイプジェスチャーによる前後日移動を想定していたが、実装ではより直感的なカルーセルUI（DateCarousel）を採用。全期間の記録日付を一覧表示し、任意の日付に直接ジャンプできるUXを提供。
