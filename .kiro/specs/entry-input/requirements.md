...
6. The 入力フォーム shall テキストが空の状態での投稿を禁止する（画像のみの投稿は不可）
7. When 投稿が正常に完了した, the システム shall タイムライン画面（/timeline）にリダイレクトする
...
4. The 入力画面 shall 1投稿あたり最大2枚の画像添付を許可する
...
5. The 下書き保存機能 shall 編集ページ (/edit/[id]) においても同様に機能する
...
6. The 編集画面 shall 既存の添付画像を削除する機能を提供する
7. The 編集画面 shall 既存の添付画像を別の画像に差し替える機能を提供する
8. When 編集が正常に完了した, the システム shall タイムライン画面（/timeline）にリダイレクトする
