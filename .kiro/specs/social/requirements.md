...
1. The システム shall 以下の4種類の達成を記録する:
   - `daily_posts`: 1日の投稿数（閾値: 20, 30, 40, 50）
   - `total_posts`: 総投稿数（閾値: 10, 30, 50, 100, 150, 200, 250, 300, 400, 500、以降100刻みで10000まで）
   - `streak_days`: 継続日数（閾値: 3, 7, 14, 30, 60, 90, 120, 150, 180, 240, 365、以降60刻みで3650日まで）
...
8. When フォロー通知を表示する, the システム shall フォローバックボタンを表示する
9. The フォローバックボタン shall 既にフォロー中の場合は「フォロー中」状態を表示する
