 ...
 *
 * 対象項目:
 * - 1.1.3: 複数画像投稿（3枚の画像を添付 → すべての画像が正しく表示）
 * - 1.1.5: Cmd+Enter送信（テキスト入力 → Cmd+Enter → 送信が実行される）
 * - 1.2.5: 編集可能時間（作成から24時間以上経過したエントリ → 編集不可の表示）
 * - 1.3.4: 削除中の状態（削除ボタン押下直後 → ローディング表示、二重クリック防止）
 */
