 ...
 *
 * 対象項目:
 * - 3.3.1: お祝い実行（未お祝いアイテムをクリック → パーティクルエフェクト、状態変更）
 * - 3.3.3: ローディング中（お祝いクリック直後 → Loader2表示、二重クリック防止）
 * - 3.4.1: フォロー中一覧（フォロー数クリック → FollowingList表示）
 * - 3.4.2: フォロワー一覧（フォロワー数クリック → FollowerList表示）
 * - 3.4.5: 空リスト（フォロー0人 → EmptyState表示）
 */
