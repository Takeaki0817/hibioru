-- bonus_hotsure（購入分のほつれ）を消費対象に追加
-- 消費優先順位: 無料配分（hotsure_remaining）→ 購入分（bonus_hotsure）

-- process_daily_streak関数を更新
  ...
  LOOP
    BEGIN
      ...
      IF NOT v_has_entry THEN
        ...
        -- 2. 購入分のほつれ（bonus_hotsure）が残っている場合は消費
...
-- コメント更新
COMMENT ON FUNCTION process_daily_streak IS '日次ストリーク判定処理（毎日0:00 JST実行想定）。bonus_hotsure（購入分）も消費対象。';
